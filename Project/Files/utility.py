...
def establish_src_conn(json_data: dict, json_section: str) -> bool:
    """establishes connection for the postgres database
       you pass it through the json"""
    try:
        
        connection_details = get_config_section(json_data["task"]["source"]["source_file_path"],\
...
def establish_mysql_conn(json_data: dict, json_section: str,json_path: str) -> bool:
    """establishes connection for the postgres database
       you pass it through the json"""
    try:
        
        connection_details = get_config_section(json_data["task"][json_section][json_path],\
        json_data["task"][json_section]["connection_name"])
...
def establish_tgt_conn(json_data: dict, json_section: str) -> bool:
    """establishes connection for the postgres database
       you pass it through the json"""
    try:
        connection_details = get_config_section(json_data["task"]["target"]["target_file_path"],\
        json_data["task"][json_section]["connection_name"])
        conn1 = sqlalchemy.create_engine(f'postgresql://{connection_details["user"]}'
        f':{connection_details["password"].replace("@", "%40")}@{connection_details["host"]}'
        f':{int(connection_details["port"])}/{connection_details["database"]}', encoding='utf-8')
