...
def qc_check(control_table_df, checks_mapping_df, check_type, ing_type, ing_loc, conn_str):
    ...
    try:
        ...
        if ing_type == 'xml':
            ...
            ge_df = ge.from_pandas(pd_df)
        elif ing_type == 'mysql_write' or ing_type == 'mysql_read':
...
def qc_pre_check(config_json_file):
    ...
    try:
        ...
        conn_str = read_config(
            config_json_file['task']['config_file'], "postgresql_1")
        conn = sqlalchemy.create_engine(f'postgresql://{conn_str["user"]}'
                f':{conn_str["password"].replace("@", "%40")}@{conn_str["host"]}')
        print("heelloooo")
        ...
        src_conn_str =  read_config(
            config_json_file['task']['source']['source_file_path'], config_json_file['task']['source'][
        ...
        if config_json_file['task']['source']['source_type'] =='csv_read':
            ...
            print(config)
        elif config_json_file['task']['source']['source_type'] =='postgres_read' or config_json_file['task']['source']['source_type'] =='mysql_read':
...
def qc_post_check(config_json_file):
    ...
    try:
        ...
        tgt_conn_str =  read_config(
            config_json_file['task']['target']['target_file_path'], config_json_file['task']['target'][
