...
def create(json_data: dict, conn, dataframe) -> bool:
    ...
    try:
        ...
        json_data["task"]["target"]["table_name"]) is False:
            ...
            logging.info("sql server ingestion completed")
        else:
...
def truncate(json_data: dict, conn: dict) -> bool:
    ...
    try:
        if db_table_exists(conn, json_data["task"]["target"]["schema"],\
            ...
            logging.info("sql server truncating table completed")
            sys.exit()
...
def drop(json_data: dict, conn: dict) -> bool:
    ...
    try:
        ...
        json_data["task"]["target"]["table_name"]) is True:
            ...
            logging.info("sql server dropping table completed")
            sys.exit()
...
def write(json_data: dict, datafram, counter: int) -> bool:
    """ function for ingesting data to sql server based on the operation in json"""
    try:
        logging.info("ingest data to sql server db initiated")
        ...
        logging.exception("ingest_data_to_sql server() is %s", str(error))
        raise Exception("ingest_data_to_sql server(): " + str(error)) from error
